...
# =============================================================================
# PLAN CONFIGURATION — defines free vs pro feature gates
# =============================================================================

PLAN_LIMITS = {
    "free": {
        "max_rows": 1000,
        "ai_calls_per_session": 3,
        "pages": ["action_center", "whats_selling"],
        "features": ["basic_clustering", "basic_pricing"],
    },
    "pro": {
        "max_rows": 50000,
        "ai_calls_per_session": -1,  # unlimited
        "pages": ["all"],
        "features": ["all"],
    },
}

# Current plan — hardcoded to "pro" until payment integration is live.
# Change to "free" to test the free tier experience.
_CURRENT_PLAN = "pro"


def _show_upgrade_prompt(context: str = "general") -> None:
    """Show a non-intrusive upgrade prompt. Context adjusts the message."""
    messages = {
        "ai_advisor": (
            "You've used your free AI questions for this session. "
            "Upgrade to Pro for unlimited AI analysis and personalized recommendations."
        ),
        "row_limit": (
            "Your file has more rows than the free plan supports. "
            "Upgrade to Pro to analyze up to 50,000 transactions at once."
        ),
        "forecast": (
            "Revenue forecasting is a Pro feature. "
            "Upgrade to see where your business is headed — and plan accordingly."
        ),
        "general": (
            "You're on the free plan. Upgrade to Pro to unlock unlimited AI analysis, "
            "advanced forecasting, and full pricing intelligence."
        ),
    }
    msg = messages.get(context, messages["general"])
    st.info(
        f"**Free plan** · {msg}\n\n"
        f"[Upgrade to Pro — $29/month](mailto:hello@kern.ai?subject=Upgrade%20to%20Pro)"
    )
...
_kern_template = go.layout.Template()
_kern_template.layout = go.Layout(
...
_kern_template.data.scatter = [
    go.Scatter(marker=dict(size=7, line=dict(width=2, color="rgba(255,255,255,0.9)")))
]

pio.templates["kern"] = _kern_template
pio.templates.default = "kern"
...
def _render_quick_price_check(df: pd.DataFrame) -> None:
    ...
    new_qty = monthly_qty * max((1 + pct) ** (-elasticity), 0) if abs(pct) > 1e-9 else monthly_qty
...
def render_best_sellers(df: pd.DataFrame, product_clusters=None):
    ...
    if agg is None:
        ...
        st.info("Upload data with at least 4 different products to see them grouped into Best Sellers, Steady Earners, Hidden Gems, and Slow Movers. Most small businesses easily pass this threshold.")
...
def render_price_intelligence(df: pd.DataFrame, product_clusters=None):
    ...
    if price_recs:
        st.markdown("---")
        st.subheader("What to change and why")
        st.caption(
    ...
    # ── Price Simulator ───────────────────────────────────────────────────
    st.markdown("---")
    st.subheader("Test a price change before you commit")
    ...
    def _sim_rev(e):
        if abs(price_change_pct) < 1e-9:
            return new_price * monthly_qty
        qty = monthly_qty * max((1 + price_change_pct) ** (-e), 0)
...
def render_growth_forecast(df: pd.DataFrame):
    ...
    if not _has_dates(df):
        st.info("Your data doesn't include dates — add a date or timestamp column to unlock revenue forecasting. Most POS exports include this automatically.")
        return

    if _CURRENT_PLAN == "free":
        _show_upgrade_prompt("forecast")
        return
    ...
    if _STATSFORECAST_AVAILABLE and len(daily) >= 10:
        try:
            ...
            if fcst_mid.sum() == 0 and daily["revenue"].sum() > 0:
                ...
                # Derive slope from historical fitted values (consistent with Prophet path)
                # Using forecast-only slope overstates trend when the model projects aggressively
                if len(daily) >= 2:
                    _hist_rev = daily["revenue"].values
                    slope = float((_hist_rev[-1] - _hist_rev[0]) / max(len(_hist_rev) - 1, 1))
                else:
                    slope = 0.0
    ...
    st.markdown("---")
    st.subheader("Which products are behind this trend?")
    ...
    if prod_forecasts:
        ...
        st.info("Add a date column and at least 4 weeks of sales history to see which specific products are driving your growth — and which ones are falling behind.")
...
def _prescribe_low_activity(
    ...
    for _, row in dead.iterrows():
        ...
        # demand elasticity 1.2 for a 20% price drop — log-log model
        new_qty = monthly_qty * (0.80 ** (-1.2))
...
def _build_action_center(df: pd.DataFrame, product_clusters) -> dict:
    ...
    for rec in price_recs:
        if rec["action"] == "↑ Raise Price" and rec["product"] in prod_lookup.index:
            ...
            margin_pct = rec.get("margin_pct") if rec.get("margin_pct") is not None and rec.get("margin_pct") != 0 else gross_margin_fallback
        ...
        if not stars.empty and not gems.empty:
            ...
            if star_cat != gem_cat and star_name != gem_name:
                ...
                quick_wins.append({
                    ...
                    "confidence": "directional",
                    "impact_range": f"~{_cur()}{bundle_upside:,.0f}/month if 1 in 10 customers add it — test first",
                    "impact_label": f"~{_cur()}{bundle_upside:,.0f}/month (estimated, 10% attach rate assumed)",
    ...
    _date_span_days = int((df["date"].max() - df["date"].min()).days) if has_dates else 0
    trend = insights.get("trend", "flat")
    if trend == "downward" and _date_span_days >= 30:
        _slope_pct = abs(insights.get("slope_pct", 0))
        _top_decliner = declining[0]["product"] if declining else None
        _decliner_note = (
...
def render_action_center(df: pd.DataFrame, product_clusters):
    ...
    if not all_actions:
        st.info("Upload your sales data to see your personalized action plan — KERN will rank the most impactful things you can do today, from price changes to product promotions.")
        return
    ...
    # ── Confidence legend ─────────────────────────────────────────────────
    with st.expander("What do the confidence badges mean?", expanded=False):
        st.markdown(
            "**🟢 Strong signal — Ready to act on**\n\n"
            "Backed by enough transaction data to be confident. You can make this change today.\n\n"
            "**🟡 Worth testing — Run a 2-week experiment**\n\n"
            "The data points in this direction, but there isn't enough variation to be precise. "
            "Try it for 2 weeks and measure the result before making it permanent.\n\n"
            "**🔴 Need more data — Keep watching**\n\n"
            "Too few transactions to draw reliable conclusions. Upload more history or wait for more sales data."
        )

    # Upgrade prompt — shown after seeing value
    if _CURRENT_PLAN == "free" and len(all_actions) > 0:
        st.markdown("---")
        _show_upgrade_prompt("general")
...
def _build_data_context(df: pd.DataFrame, product_clusters) -> str:
    ...
    if _has_dates(df):
        ...
        if not by_hour.empty and not by_day.empty:
            ...
            lines.append(f"\nPEAK TRADING TIMES:")
            # Only report busiest hour if the data actually has time variation
    ...
    if _MLXTEND_AVAILABLE and _has_dates(df):
        _df_hash = hashlib.md5(pd.util.hash_pandas_object(df).values.tobytes()).hexdigest()
        if st.session_state.get("_basket_rules_hash") == _df_hash and "_basket_rules_cache" in st.session_state:
            _, basket_rules, basket_err, _bm = st.session_state["_basket_rules_cache"]
        else:
            _, basket_rules, basket_err, _bm = _compute_basket_rules(df.to_csv(index=False))
            st.session_state["_basket_rules_hash"] = _df_hash
            st.session_state["_basket_rules_cache"] = (_, basket_rules, basket_err, _bm)
...
def render_monthly_report_section(df: pd.DataFrame, product_clusters) -> None:
    ...
    if st.button("Write my summary report", key="gen_report_btn", type="primary"):
    ...
    with col2:
        if st.button("📋 Copy report text", key="copy_report_btn"):
...
def render_ai_advisor(df: pd.DataFrame, product_clusters=None):
    st.header("Ask anything about your business")
    ...
    _data_id = hashlib.md5(pd.util.hash_pandas_object(df).values.tobytes()).hexdigest()[:10]
    ...
    if not st.session_state.chat_messages:
        ...
        for q in [
            ...
            "Which product should I promote and why?",
            "Am I leaving money on the table anywhere?",
            "When are my busiest hours and should I adjust staffing?",
            "Which of my slow-moving items are worth keeping?",
        ]:
            st.markdown(f"- *{q}*")
    ...
    # Silently skip if no API key — show gentle notice instead of dead-end
    if model is None:
        if _CURRENT_PLAN == "free":
            _show_upgrade_prompt("ai_advisor")
        else:
            st.info("AI Advisor is available when an API key is configured. You can still explore all other pages.")
        return
    ...
    if user_input := st.chat_input("Ask anything about your business..."):
        ...
        # Build Groq conversation history (OpenAI format)
        # System message carries the business context
        groq_history = [
    ...
    # Clear chat button
    if st.session_state.chat_messages:
        if st.button("Start a new conversation", key="clear_chat"):
            st.session_state.chat_messages = []
            st.rerun()

    if st.session_state.get("_last_ai_error") and os.environ.get("DEBUG_MODE", ""):
...
def render_pricing_page(df: pd.DataFrame = None, product_clusters=None):
    st.header("Simple pricing, powerful results")
    st.caption("Choose the plan that fits your business")

    st.markdown(
        "<p style='font-family:Raleway,sans-serif;font-size:1.05rem;color:#1e3a5f;"
        "line-height:1.8;max-width:600px;margin-bottom:2rem;'>"
        "Businesses using KERN find an average of 3 immediate actions to take in their first session — "
        "from price adjustments to product promotions that pay for themselves."
        "</p>",
        unsafe_allow_html=True,
    )
    ...
    with col_free:
        st.markdown(
            "<div style='background:#f0f4f8;border:2px solid #e2e8f0;border-radius:16px;"
            "padding:2rem;text-align:center;'>"
            "<p style='font-size:0.7rem;font-weight:700;text-transform:uppercase;"
            "letter-spacing:0.2em;color:#64748b;margin:0;'>FREE</p>"
            "<p style='font-family:Cormorant,serif;font-size:3rem;font-weight:500;"
            "color:#1e3a5f;margin:0.5rem 0;'>$0</p>"
            "<p style='font-size:0.85rem;color:#64748b;margin:0 0 1.5rem;'>per month</p>"
            "<div style='text-align:left;font-size:0.9rem;color:#1e3a5f;line-height:2.2;'>"
            "✓ Up to 1,000 transactions<br>"
            "✓ Action Center<br>"
            "✓ Product clustering (Best Sellers)<br>"
            "✓ Basic pricing analysis<br>"
            "✓ 3 AI questions per session<br>"
            "✗ <span style='color:#94a3b8;'>Revenue forecasting</span><br>"
            "✗ <span style='color:#94a3b8;'>Market basket analysis</span><br>"
            "✗ <span style='color:#94a3b8;'>Unlimited AI Advisor</span><br>"
            "✗ <span style='color:#94a3b8;'>Monthly summary reports</span>"
            "</div>"
            "</div>",
            unsafe_allow_html=True,
        )
        st.markdown("")  # spacer
        st.button("Current plan", disabled=True, key="free_plan_btn", use_container_width=True)

    with col_pro:
        st.markdown(
            "<div style='background:#1e3a5f;border:2px solid #2563eb;border-radius:16px;"
            "padding:2rem;text-align:center;'>"
            "<p style='font-size:0.7rem;font-weight:700;text-transform:uppercase;"
            "letter-spacing:0.2em;color:#93c5fd;margin:0;'>PRO</p>"
            "<p style='font-family:Cormorant,serif;font-size:3rem;font-weight:500;"
            "color:#f0f4f8;margin:0.5rem 0;'>$29</p>"
            "<p style='font-size:0.85rem;color:#93c5fd;margin:0 0 1.5rem;'>per month</p>"
            "<div style='text-align:left;font-size:0.9rem;color:#f0f4f8;line-height:2.2;'>"
            "✓ Up to 50,000 transactions<br>"
            "✓ Everything in Free<br>"
            "✓ Revenue forecasting<br>"
            "✓ Market basket analysis<br>"
            "✓ Unlimited AI Advisor<br>"
            "✓ Monthly summary reports<br>"
            "✓ Price simulator<br>"
            "✓ Period-over-period comparison<br>"
            "✓ Priority support"
            "</div>"
            "</div>",
            unsafe_allow_html=True,
        ...
        st.link_button(
            "Upgrade to Pro",
            url="mailto:hello@kern.ai?subject=Upgrade%20to%20Pro&body=I'd%20like%20to%20upgrade%20my%20KERN%20account%20to%20Pro.",
            use_container_width=True,
            type="primary",
        )
    ...
    st.markdown("---")
    st.markdown(
        "<p style='text-align:center;font-size:0.85rem;color:#64748b;max-width:500px;margin:1rem auto;'>"
        "Questions? Email <a href='mailto:hello@kern.ai' style='color:#2563eb;'>hello@kern.ai</a>"
        "</p>",
        unsafe_allow_html=True,
    )
...
def main():
    ...
    st.markdown(
        ...
        "Your sales data, turned into decisions</p>",
        unsafe_allow_html=True,
    )
...
👈 Click <strong>Coffee Shop</strong> or <strong>Retail Store</strong> demo, or upload your own file from the sidebar
    ...
    if demo_mode and not uploaded:
        ...
        if len(raw_df) > MAX_ROWS:
            st.error(
                f"This file has {len(raw_df):,} rows — KERN works best with up to {MAX_ROWS:,}. "
    ...
    with st.spinner("Analyzing your product mix..."):
    ...
    if st.session_state.get("_show_anomaly_history"):
        ...
        for name, fn in chapters:
            if choice == name and fn is not None:
                if name in ("Action Center", "Pricing", "Business Overview", "What's Selling", "AI Advisor", "Pricing & Plans"):
