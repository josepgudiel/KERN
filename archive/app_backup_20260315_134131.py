"""
KERN — Smart insights for small business
...
st.set_page_config(page_title="KERN", page_icon="📊", layout="wide")
...
_INDUSTRY_TEMPLATES: dict = {
    ...
    "Restaurant / Bar": {
        ...
        "primary_metrics":  ["revenue", "avg_order_value", "peak_hours"],
        "surface_first":    ["peak_hours", "declining_products", "staffing"],
        "collapse_first":   ["forecast", "basket"],
        "action_center_emphasis": ["staffing", "declining", "wow"],
        "ai_hint": (
    ...
    "Café / Coffee Shop": {
        ...
        "surface_first":    ["best_sellers", "rising_stars", "peak_hours"],
    ...
    "E-commerce": {
        ...
        "collapse_first":   ["peak_hours", "staffing"],
    ...
    "Services": {
        "label": "Services",
        "primary_metrics":  ["revenue", "avg_order_value", "peak_hours"],
        "surface_first":    ["peak_hours", "best_sellers"],
        "collapse_first":   ["basket", "forecast"],
        "action_center_emphasis": ["pricing", "rising", "staffing"],
        "ai_hint": (
...
def _tpl_expanded(section_key: str) -> bool:
    """Return True if this section should be expanded by default for the current industry template."""
    try:
    ...
    for attempt in range(retries + 1):
        try:
            ...
            if "429" in str(e) and attempt < retries:
                st.toast("Rate limit hit — retrying in 15 seconds...", icon="⏳")
                time.sleep(15)
                continue
            raise
    ...
    try:
        ...
        if name.endswith(".xls"):
            try:
                ...
                st.error("Install xlrd to read .xls files: pip install xlrd")
                return None
...
def _render_ai_brief_expander(
    ...
    *,
    ...
    with st.expander("🤖 AI Business Brief — 3 Priorities for This Week", expanded=True):
        ...
        if brief is None:
            if st.button("Generate AI Brief", key=button_key):
                with st.spinner("Generating your business brief..."):
                    brief = _generate_ai_brief(data_ctx)
                    st.session_state[brief_cache_key] = brief
            else:
                st.caption("Click to generate AI priorities for this week.")
                return

        if brief and "error" in brief:
            if brief["error"] == "api_key":
                st.info(
                    "To unlock AI-generated priorities, add your Groq API key: "
                    "`GROQ_API_KEY=your_key_here` and restart the app. "
                    "Get a free key at console.groq.com — then visit **AI Advisor** to chat with your data."
                )
            else:
                st.warning(f"Brief unavailable: {brief['error']}")
        elif brief:
...
def _render_trend_anomaly_section(df: pd.DataFrame) -> None:
    ...
    if not insights["has_dates"]:
        return
    st.markdown("---")
    st.subheader("How Your Business is Trending")
            ...
            for a in insights["anomalies"]:
                ...
                if a["direction"] == "spike":
                    ...
                    action_tip = (
                        ...
                        "if so, run a targeted promotion on that day (loyalty double-stamps, happy-hour deal)."
                    )
...
def _render_quick_price_check(df: pd.DataFrame) -> None:
    ...
    st.caption("→ Open **Pricing** in the sidebar for full pricing analysis across all products.")
...
def _render_cluster_cards(agg: pd.DataFrame) -> None:
    ...
    with st.expander("See full chart", expanded=False):
        ...
        st.caption(
            ...
            "(e.g. only on the menu for 2–3 weeks) may appear in Low Activity simply because there wasn't "
...
def _render_rising_stars_section(df: pd.DataFrame) -> None:
    ...
    with st.expander("Rising Stars — filter thresholds", expanded=_tpl_expanded("rising_stars")):
    ...
    if rising is None:
        return
    st.markdown("---")
    st.subheader("🚀 Products on the Rise")
    ...
    st.markdown("**💡 Suggested Actions for Rising Stars**")
    for _, row in rising.iterrows():
        ...
        st.write(
            ...
            f"Feature it as a daily special, ensure you never stock out, and put it in a visible menu/shelf position. {urgency}"
        )
...
def _render_declining_products_section(df: pd.DataFrame) -> None:
    ...
    if not declining:
        return
    st.markdown("---")
    st.subheader("📉 Items Losing Ground")
    st.caption(
...
def _render_cluster_action_plan(agg: pd.DataFrame) -> None:
    ...
    with st.expander("📋 Specific Action Plan", expanded=_tpl_expanded("best_sellers")):
        st.caption("Concrete next steps for each product category — act on these this week.")
        for cat in ["Stars", "Cash Cows", "Hidden Gems", "Low Activity"]:
            ...
            if cat == "Stars":
                steps = [
                    f"Launch a loyalty punch card for **{top_name}** — your #1 Star at {cur_sym}{top_rev:,.0f} revenue. Reward 5 purchases, get 1 free. Stars already have the volume to make this self-funding.",
                    f"Never let **{top_name}** or **{second_name}** run out of stock — they drive the most revenue. Set a reorder alert at 20% remaining inventory.",
                    f"Feature **{top_name}** in the first position on your menu/shelf — best presentation, eye-level placement. First-position items lift 15–20% just from visibility.",
                ]
            elif cat == "Cash Cows":
                ...
                steps = [
                    f"Bundle **{top_name}** with **{gem_name}** at a small discount ('Add {gem_name} for just {cur_sym}2 more') — Cash Cows have the volume to make cross-sells automatic.",
                    f"Train staff to mention **{top_name}** and **{second_name}** by name when upselling — scripted recommendations convert 30–40% better than generic suggestions.",
                    f"**{top_name}** is a high-margin workhorse — avoid discounting it. Protect the price point and instead bundle it with lower-margin items to lift average order value.",
                ]
            elif cat == "Hidden Gems":
                steps = [
                    f"Feature **{top_name}** as this week's 'Staff Pick' or 'Daily Special' — it has strong unit economics but low awareness. A rotating spotlight is the cheapest promotion you have.",
                    f"Add a menu callout for **{top_name}** that explains *why* it's special (origin, preparation, what makes it different). Storytelling sells — especially for hidden items.",
                    f"If **{top_name}** has been on the menu 60+ days without breaking through, try a 2-week feature at a promotional price, then evaluate. Don't remove it without testing visibility first.",
                ]
            else:  # Low Activity
                steps = [
                    f"⚠️ Before acting on **{top_name}** ({top_qty} units sold), check how long it's been on the menu — a product available fewer than 30 days may simply need more time.",
                    f"Give **{top_name}** a low-cost visibility boost — feature it as a daily special or have staff mention it unprompted — before deciding to remove it. Removal has hidden costs (menu reprinting, waste of existing inventory).",
                    f"If **{top_name}** has been available long-term with consistent low sales, run a 2-week promotion to test demand, then make a final call. Data from the promotion is more reliable than data from passive availability.",
                ]
...
def render_best_sellers(df: pd.DataFrame, product_clusters=None):
    ...
    if agg is None:
        ...
        st.warning("Need at least 4 products for clustering.")
...
# =============================================================================
# 3. TIME SERIES ANALYSIS — Peak Hours Heatmap
# =============================================================================

def render_peak_hours(df: pd.DataFrame):
    st.header("When to Staff?")
    st.caption("Understand your busiest times so you can schedule smarter — and stop paying for slow hours.")
    ...
    if _has_dates(df):
        ...
        flat = heat.stack().reset_index()
        flat.columns = ["day", "hour", "revenue"]

        # ── 1. Plain-text peak insight — shown first ───────────────────────
        if not flat.empty and flat["revenue"].sum() > 0:
            best = flat.loc[flat["revenue"].idxmax()]
            st.info(
                f"**Your busiest time is {best['day']} around {int(best['hour']):02d}:00.** "
                f"This is when you need your most experienced staff on the floor."
            )

        # ── 2. Staffing & Operations Recommendations ───────────────────────
        tips = _staffing_from_heatmap(heat)
        if tips:
            st.markdown("---")
            st.subheader("🧑‍🍳 What to Do")
            st.caption("Specific actions based on your actual sales patterns — use these to build your schedule.")
            for tip in tips:
                st.success(tip)

        # ── 3. Day-part revenue breakdown (always visible) ─────────────────
        morning_r = flat[flat["hour"].between(6, 11)]["revenue"].sum()
        lunch_r = flat[flat["hour"].between(12, 14)]["revenue"].sum()
        afternoon_r = flat[flat["hour"].between(15, 17)]["revenue"].sum()
        evening_r = flat[flat["hour"].between(18, 22)]["revenue"].sum()
        daypart_data = {
            "Time of Day": ["Morning (6–11am)", "Lunch (12–2pm)", "Afternoon (3–5pm)", "Evening (6–10pm)"],
            "Revenue": [morning_r, lunch_r, afternoon_r, evening_r],
        }
        dpdf = pd.DataFrame(daypart_data).query("Revenue > 0").sort_values("Revenue", ascending=False)
        if not dpdf.empty:
            st.markdown("**Revenue by Time of Day**")
            total_dp = dpdf["Revenue"].sum()
            dpdf["Share"] = (dpdf["Revenue"] / total_dp * 100).round(1).astype(str) + "%"
            st.dataframe(
                dpdf.style.format({"Revenue": f"{_cur()}{{:,.2f}}"}),
                use_container_width=True, hide_index=True,
            )

        # ── 4. Heatmap — behind expander ───────────────────────────────────
        with st.expander("See full hourly breakdown", expanded=_tpl_expanded("peak_hours")):
            fig = px.imshow(
                heat, labels=dict(x="Hour of Day", y="Day of Week", color="Revenue"),
        ...
        st.warning("No date/timestamp column found. Upload data with dates to see peak hours.")
...
def _estimate_product_elasticity(df: pd.DataFrame, product: str) -> tuple:
    ...
    if price_cv < 0.03:
        return None, None, None, (
            f"insufficient price variation (CV={price_cv:.1%}, need ≥3%) — "
    ...
    if dof < 3:
        return None, None, None, (
            f"insufficient data — only {n_pts} price bins retained (dof={dof}, need dof ≥ 3). "
    ...
    if t_stat < t_crit or r2 < 0.20:
        return None, None, None, (
            f"price–quantity relationship not distinguishable from noise "
            f"(t={t_stat:.1f}, t_crit={t_crit:.2f} at dof={dof}, R²={r2:.2f}) — "
    ...
    note = (
        ...
        f"{n_pts} price bins (elasticity={elasticity:.2f}, R²={r2:.2f}, fit: {fit_quality}){cap_note}"
    )
...
def render_price_intelligence(df: pd.DataFrame, product_clusters=None):
    ...
    fig.add_trace(
        go.Bar(x=by_bucket["price_bucket"], y=by_bucket["total_revenue"], name="Revenue"),
        row=1, col=1
    )
    fig.add_trace(
        go.Bar(x=by_bucket["price_bucket"], y=by_bucket["total_qty"], name="Quantity"),
        row=2, col=1
    )
    fig.update_layout(height=450, showlegend=False)
    st.plotly_chart(fig, use_container_width=True)
    
    st.subheader("What You're Charging per Item")
    cur = _cur()
    st.dataframe(
    ...
    ols_strong = est_e is not None and "fit: strong" in est_note
    ...
    if not ols_strong:
        st.info(
            ...
            f"Estimate based on: *{elas_source}* "
    ...
    if abs(price_change_pct) > 0.01:
        ...
        with st.expander("See the numbers behind this estimate"):
            ...
            st.caption(f"Elasticity source: {elas_source}")
            scenario_df = pd.DataFrame({
                "Scenario": ["Pessimistic", "Expected", "Optimistic"],
                "Elasticity used": [f"{elas_high:.2f}", f"{elasticity:.2f}", f"{elas_low:.2f}"],
...
def render_growth_forecast(df: pd.DataFrame):
    ...
    if use_prophet:
        ...
        try:
            m = _Prophet(
                ...
                changepoint_prior_scale=0.05 if date_span < 30 else (0.10 if date_span < 60 else 0.15),
            ...
            # ── Optional cross-validation to show accuracy ──────────────────
            if len(daily) >= 60 and st.checkbox("Show model accuracy (cross-validation)", value=False, key="cv_check"):
                from prophet.diagnostics import cross_validation, performance_metrics
                initial_days = max(30, int(len(daily) * 0.6))
                horizon_days = min(14, max(7, int(len(daily) * 0.15)))
                _cv_key = f"prophet_cv_{len(daily)}_{initial_days}_{horizon_days}"
                if _cv_key not in st.session_state:
                    with st.spinner("Running cross-validation — this takes ~10 s…"):
                        try:
                            ...
                            st.session_state[_cv_key] = None
                cv_result = st.session_state.get(_cv_key)
                if cv_result is not None:
                    mape_val, rmse_val, mae_val = cv_result
                    cv1, cv2, cv3 = st.columns(3)
                    cv1.metric("MAPE", f"{mape_val:.1f}%",
                               help="Mean Absolute % Error — lower is better. <10% = excellent, <20% = good")
                    cv2.metric("MAE", f"{_cur()}{mae_val:,.2f}",
                               help="Mean Absolute Error in dollars")
                    cv3.metric("RMSE", f"{_cur()}{rmse_val:,.2f}",
                               help="Root Mean Square Error — penalises large misses more heavily")
                else:
                    st.caption("Cross-validation failed — need more data points.")
    ...
    if not use_prophet and not use_statsforecast:
        ...
        try:
            slope, intercept = np.polyfit(daily["days_since_start"], daily["revenue"], 1)
        except Exception:
        ...
        if _PROPHET_AVAILABLE:  # noqa: SIM108
            ...
            method_note = "Linear trend + day-of-week seasonal. Run `pip install prophet` for higher accuracy."

    # ── Chart ─────────────────────────────────────────────────────────────────
    fig = go.Figure()
    fig.add_trace(go.Scatter(
        x=daily["date_only"], y=daily["revenue"],
        name="Daily revenue", line=dict(color="#93c5fd", width=1), opacity=0.5
    ))
    fig.add_trace(go.Scatter(
        x=daily["date_only"], y=daily["rolling_7"],
        name="7-day avg", line=dict(color="#1e3a5f", width=2.5)
    ))
    fig.add_trace(go.Scatter(
    ...
    fig.add_trace(go.Scatter(
        ...
        # Label clearly as variability range — NOT a statistical confidence interval
        name="Likely range — actual may vary"
    ))
    fig.add_trace(go.Scatter(
        x=future_dates, y=fcst_mid,
        name="Forecast (trend)", line=dict(color="#2563eb", width=2, dash="dash")
    ))
    fig.update_layout(height=450, xaxis_title="Date", yaxis_title=f"Revenue ({_cur()})", showlegend=True)
    ...
    if prod_forecasts:
        ...
        if chart_items:
            ...
            fig_multi.update_layout(
                ...
                xaxis_title="Week", yaxis_title=f"Revenue ({_cur()})",
    ...
    actions = _growth_actions(trend_key, df=df)

    st.markdown("---")
    st.subheader("📋 Growth Action Plan")
    trend_context = {
    ...
    if trend_key == "upward":
        ...
        flag_cols[0].warning("**Risk:** If the decline continues for 3+ weeks, it may reflect a structural issue (competition, seasonality, menu fatigue) — not just a slow week.")
    ...
    for _, row in agg.iterrows():
        ...
        if qty >= qty_high_threshold and price <= price_low_threshold and n_txn >= MIN_TXN_FOR_RAISE:
            ...
            if _e_used is not None:
                ...
                rev_signal = f"directional revenue signal: {cur}{adj_qty * sug - qty * price:+,.0f} (elasticity={_e_used:.2f}, test before acting)"
            else:
                rev_signal = "elasticity not estimable — treat as a directional suggestion only"
            reason = (
                f"High demand relative to peers ({int(qty)} units, above the 65th percentile) "
                f"with a below-average price (bottom 35%). A small price increase may be "
        ...
        # ── High price, low demand → suggest a modest test reduction ─────
            ...
            reason = (
                f"Priced above the 65th percentile but selling below the 35th percentile "
...
def _staffing_from_heatmap(heat: pd.DataFrame) -> list:
    """Natural-language staffing & operations tips from the day×hour heatmap."""
    if heat.empty:
        return []
    flat = heat.stack().reset_index()
    flat.columns = ["day", "hour", "revenue"]
    flat = flat[flat["revenue"] > 0]
    if flat.empty:
        return []
    by_hour = flat.groupby("hour")["revenue"].sum().sort_values(ascending=False)
    by_day = flat.groupby("day")["revenue"].sum().sort_values(ascending=False)
    top_hours = by_hour.head(3).index.tolist()
    slow_hours = by_hour.tail(3).index.tolist()
    top_days = by_day.head(2).index.tolist()
    slow_day = by_day.index[-1] if len(by_day) > 1 else None

    morning = flat[flat["hour"].between(6, 11)]["revenue"].sum()
    lunch = flat[flat["hour"].between(12, 14)]["revenue"].sum()
    afternoon = flat[flat["hour"].between(15, 17)]["revenue"].sum()
    evening = flat[flat["hour"].between(18, 22)]["revenue"].sum()
    parts = {"Morning (6–11am)": morning, "Lunch (12–2pm)": lunch,
             "Afternoon (3–5pm)": afternoon, "Evening (6–10pm)": evening}
    active = {k: v for k, v in parts.items() if v > 0}
    peak_part = max(active, key=active.get) if active else None
    slow_part = min(active, key=active.get) if active else None

    tips = []
    top_str = ", ".join(f"{h}:00" for h in top_hours)
    tips.append(f"**Full team on deck:** Your busiest hours are **{top_str}** — have everyone in and prep done before these windows open.")

    slow_str = " and ".join(f"{h}:00" for h in slow_hours[:2])
    tips.append(f"**Trim labor costs:** Foot traffic is lowest around **{slow_str}** — ideal for shift overlaps or reduced staffing without impacting service.")

    if top_days:
        days_str = " and ".join(top_days)
        tips.append(f"**Pre-shift planning:** **{days_str}** generate the most revenue — stock up and confirm full-team schedules the evening before.")

    if slow_day:
        tips.append(f"**{slow_day} boost:** This is your quietest day. A weekly special or loyalty double-stamp promotion can pull in regulars who normally skip it.")

    if peak_part:
        tips.append(f"**Day-part focus:** Your **{peak_part}** window drives the most revenue — ensure your best sellers are freshly prepped and visible before this rush starts.")

    if slow_part:
        tips.append(f"**Off-peak opportunity:** The **{slow_part}** slot is underperforming. A limited-time deal or staff upsell coaching during this window can unlock idle capacity.")

    return tips


def _growth_actions(trend: str, df: pd.DataFrame | None = None) -> list:
    ...
    # Extract real names to make tips specific
    top_product = ""
    second_product = ""
    bottom_product = ""
    ...
    if trend == "upward":
        return [
            f"**Double down on {top_ref}:** With revenue growing, introduce a new item adjacent to {top_ref} — customers are already in a buying mood. Bundle or upsell first, then introduce.",
            f"**Test a price increase on {second_ref}:** Growth periods absorb price changes more easily. Try a 5–8% increase on {second_ref} and watch volume for 2 weeks — if volume holds, you found free margin.",
            f"**Build loyalty around {top_ref}:** Capture customers while traffic is up — a punch card or digital stamp program tied to {top_ref} is cheap and extends the growth curve.",
            f"**Lock in supply for {top_ref}:** Rising demand means stockouts hurt more. Confirm inventory and supplier lead times so {top_ref} never runs out during a peak.",
        ]
    elif trend == "downward":
        return [
            f"**Run a targeted promo on {top_ref}:** A 3-day flash sale or 'bring a friend' deal on {top_ref} can interrupt a slide quickly — act this week while it's recoverable.",
            f"**Trim {bottom_ref} from your menu:** Declining revenue is worsened by the hidden cost of low-selling items (prep time, waste, complexity). Cutting {bottom_ref} reduces complexity without losing meaningful revenue.",
            f"**Drive repeat visits with {top_ref}:** It costs 5× more to acquire a new customer than retain one. A 'come back this week' incentive tied to {top_ref} is your highest-ROI move right now.",
            f"**Audit peak staffing:** Understaffing during busy hours loses {top_ref} sales silently. Check whether service quality has dropped during your peak windows.",
        ]
    else:
        return [
            f"**Feature {top_ref} prominently for 2 weeks:** Flat = stable, not growing. Giving {top_ref} first-position visibility on your menu/shelf can break the plateau — track the lift.",
            f"**Lift average order value with {top_ref}:** With steady traffic, the easiest win is a {cur}1–2 add-on when {top_ref} is ordered — a smaller complement item or upgrade suggestion.",
            f"**Create off-peak urgency:** A structured happy-hour discount unlocks revenue from slow windows without cannibalizing {top_ref} peak sales.",
            f"**Invest in reviews for {top_ref}:** Flat growth often signals weak new-customer discovery. A Google/Yelp review push featuring {top_ref} is free and compounds over months.",
        ]
...
def render_market_basket(df: pd.DataFrame):
    ...
    if not _MLXTEND_AVAILABLE:
        st.warning("Run `pip install mlxtend` to enable market basket analysis.")
        return

    if not _has_dates(df):
        st.warning("A date column is required for basket analysis.")
        return
    ...
    if err:
        msg_map = {
            "mlxtend_missing": "Run `pip install mlxtend` to enable this page.",
            "no_date": "Date column required for basket analysis.",
            "no_frequent_itemsets": "No frequent product combinations found. Try uploading data with more transaction days.",
            "no_rules": "No association rules met the minimum lift threshold. Your products may sell independently.",
            "no_single_consequent_rules": "All discovered rules have multiple-item consequents — no simple 'A → B' bundles found.",
        }
        st.info(msg_map.get(err, f"Basket analysis unavailable: {err}"))
        return
    ...
    if not two_item.empty:
        ...
        fig_hm = px.imshow(
            matrix,
            title="Product Co-occurrence (% of days both appear)",
            color_continuous_scale="Blues",
            labels=dict(color="Support %"),
        )
        fig_hm.update_layout(height=420)
        st.plotly_chart(fig_hm, use_container_width=True)

    # ── Rules table ───────────────────────────────────────────────────────────
    st.subheader("Items That Often Sell Together")
    if _using_proxy:
        st.caption(
    ...
    display.columns = ["Item A", "Item B", "% Together", "% When A, also B", "Strength"]
    ...
    display["Strength"]         = display["Strength"].round(2)
    ...
    if _using_proxy:
        ...
        st.caption("Concrete actions for your top pairings — tested bundle deals that lift average order value.")
        ...
        for idx, rule in _rules_ranked.head(8).iterrows():
            ...
            with st.expander(f"🔗 **{ant}** + **{cons}** — {lift:.2f}× lift"):
                col_text, col_kpi = st.columns([4, 1])
                with col_text:
                    st.write(
                        f"Customers who buy **{ant}** also buy **{cons}** with "
                        f"**{conf_pct:.0f}% confidence** — {lift:.1f}× more likely than random co-occurrence."
                    )
                    if p1 > 0 and p2 > 0:
    ...
    with st.expander("📖 What do these numbers mean?"):
        st.write(
            ...
            "**Strength** = how much more often they appear together than you'd expect by chance. "
            "A strength of 2.0 means customers buy them together twice as often as random chance. "
        ...
        if not stars.empty and not gems.empty:
            ...
            quick_wins.append({
                ...
                "detail": (
                    ...
                    f"Feature as 'Staff Pick', add a menu callout, or run a 1-week promotion."
                ),
                "impact_dollars": None,
            ...
            if prescription is not None:
                ...
                watch_outs.append({
                    ...
                    "detail": (
                        ...
                        "long each item has been on the menu — something new may just "
    ...
    if trend == "downward":
        watch_outs.append({
            ...
            "detail": (
                ...
                "competition, menu fatigue, or a seasonal shift. Review your top products and check "
...
def render_period_comparison(comparison: dict) -> None:
    ...
    st.markdown(
        "<h3 style='margin-bottom:0.3rem;'>Period Comparison</h3>",
        unsafe_allow_html=True,
    )
...
def render_action_center(df: pd.DataFrame, product_clusters):
    st.header("Your Business To-Do List")
    st.caption("The most important things to act on today, ranked by estimated impact on your revenue.")
    ...
    for i, action in enumerate(all_actions, 1):
        ...
        with col_main:
            ...
            if action.get("impact_range"):
                card_body += f"\n\n*Estimate: {action['impact_range']}*"
            elif action.get("impact_label"):
                card_body += f"\n\n*{action['impact_label']}*"
            if is_risk:
        ...
        with col_badge:
            ...
            conf_label, conf_emoji = _conf_display.get(conf_key, ("Directional", "🟡"))
            n_tx = action.get("n_transactions")
            tx_line = (
                f"<div style='font-size:0.6rem;color:#64748b;margin-top:0.2rem;'>"
                f"{n_tx:,} sale{'s' if n_tx != 1 else ''}</div>"
    ...
    st.caption(
        ...
        "🟡 **Directional** — data points this way, but not enough variation to be precise; run a short test before committing. "
    ...
    if (
        ...
        st.info(
            f"📋 **{unannotated} anomalies in your history have no notes yet.** "
            "Adding a note takes 5 seconds — and builds a record you'll thank yourself for later. "
            "Open **Anomaly History** in the sidebar."
        )
...
def render_monthly_report_section(df: pd.DataFrame, product_clusters) -> None:
    ...
    st.markdown(
        ...
        f"text-transform:uppercase;margin-bottom:1.2rem;'>Generated by KERN</div>"
        ...
        f"text-align:right;letter-spacing:0.15em;'>KERN — kern.ai</div>"
        f"</div>",
        unsafe_allow_html=True,
    )
...
# =============================================================================
# AI ADVISOR — Streaming chat with Claude about the business data
# =============================================================================
...
def _detect_industry_from_data(df: pd.DataFrame) -> str:
    """Pure heuristic industry detection from product names. Never raises."""
    try:
        products_lower = df["product"].str.lower().str.cat(sep=" ")

        if any(k in products_lower for k in [
            "espresso", "latte", "cappuccino", "americano", "flat white",
            "cold brew", "macchiato", "cortado",
        ]):
            return "café"
        elif any(k in products_lower for k in [
            "burger", "pizza", "pasta", "entree", "entrée", "starter",
            "main course", "side dish", "dessert", "appetizer",
        ]):
            return "restaurant"
        elif any(k in products_lower for k in [
            "croissant", "muffin", "loaf", "sourdough", "brioche",
            "éclair", "tart", "scone",
        ]):
            return "bakery"
        elif any(k in products_lower for k in [
            "t-shirt", "jeans", "dress", "jacket", "sneakers", "blouse",
            "hoodie", "cardigan",
        ]):
            return "clothing retail"
        elif any(k in products_lower for k in [
            "shampoo", "conditioner", "facial", "massage", "manicure",
            "wax", "blowout",
        ]):
            return "beauty / salon"
        return ""
...
def render_ai_advisor(df: pd.DataFrame, product_clusters=None):
    ...
    if model is None:
        st.info(
            "To unlock the AI Advisor, add your Groq API key: "
            "`GROQ_API_KEY=your_key_here` and restart the app. "
...
def main():
    ...
    # Demo data button
    if st.sidebar.button("▶ Try Demo Data (Coffee Shop)", use_container_width=True):
    ...
    if uploaded:
        ...
        if uploaded.size > MAX_UPLOAD_MB * 1024 * 1024:
            st.sidebar.error(f"File too large ({uploaded.size / 1024 / 1024:.1f} MB). Please upload a file under {MAX_UPLOAD_MB} MB.")
            uploaded = None
        else:
...
KERN turns your existing transaction data into clear, actionable decisions —
what to sell more of, when to run promotions, what to cut from your menu, and where
your revenue is actually coming from. No guesswork. No spreadsheet diving. Just answers.
</p>
</div>

<p style='font-family:Raleway,sans-serif;font-size:0.72rem;font-weight:700;text-transform:uppercase;letter-spacing:0.25em;color:#2563eb;margin-bottom:1.2rem;'>What you'll see — 8 pages, no clutter</p>
...
<div style='background:#e2e8f0;border:1px solid #93c5fd;border-radius:12px;padding:1.4rem;'>
<p style='font-family:Cormorant,serif;font-size:1.2rem;font-weight:500;color:#1e3a5f;margin:0;'>🕐 When to Staff</p>
<p style='font-family:Raleway,sans-serif;font-size:0.95rem;color:#1e3a5f;margin:0.5rem 0 0;line-height:1.6;'>See your busiest hours and days, with specific staffing recommendations you can act on this week.</p>
</div>
...
<p style='font-family:Raleway,sans-serif;font-size:0.95rem;color:#e2e8f0;margin:0.5rem 0 0;line-height:1.6;'>Ask anything about your data. Powered by Claude — answers like a consultant who has read every row.</p>
</div>
...
👈 Click <strong>Try Demo Data</strong> or upload your own file from the sidebar
</p>
</div>
        """, unsafe_allow_html=True)
        return
    ...
    if demo_mode and not uploaded:
        ...
        if st.session_state.get("_last_file_hash") != file_hash:
            for _k in ["map_product", "map_qty", "map_rev", "map_up", "map_cost", "map_date", "map_loc", "map_txn",
                       "chat_history", "data_context", "data_context_hash"]:
    ...
    chapters = [
        ...
        ("Forecast",           render_growth_forecast),
        ("What Sells Together", render_market_basket),
        ("When to Staff",      render_peak_hours),
        ("AI Advisor",         render_ai_advisor),
        ("📋 Anomaly History", None),
    ]
...
def render_anomaly_history() -> None:
    st.header("Sales Anomaly History")
    st.caption(
        "Unusual days detected across all your uploads. "
    ...
    for entry in filtered:
        ...
        with right_col:
            st.markdown(
                ...
                f"<span style='font-weight:700;font-size:1.1rem;color:{z_color};'>{z:.1f}σ</span>"
                f"</div>",
                unsafe_allow_html=True,
            )
